...
export const CurrentAffiliatesContext = React.createContext<CurrentAffiliatesContextType>({
  currentAffiliates: [],
...
export const CurrentAffiliatesProvider: React.FC<Props> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (affiliates.length) {
      newList = newList.filter(affiliateRow => affiliates.includes(affiliateRow.Affiliate))
    }

    if (query) {
      const normalizedQuery = query.toLowerCase();

      newList = newList.filter(affiliate => affiliate.Affiliate.toLowerCase().includes(normalizedQuery))
    }
      ...
      if (sort) {
        newList.sort((firstAffiliate, secondAffiliate) => {
          ...
          if (Object.values(SortOptions).includes(sort)) {
            return (order ? 1 : -1)
              * (firstAffiliate[sort] - secondAffiliate[sort]);
          }
