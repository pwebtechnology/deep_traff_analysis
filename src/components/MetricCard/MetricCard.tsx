...
export const MetricCard: React.FC<Props> = ({ currentMetric, totalMetric, prevDayMetric, isTotal }) => {
  const { searchParams } = useContext(CurrentAffiliatesContext);

  const affiliates: string[] = searchParams.getAll('affiliates') as string[] || [];
  ...
  const compareOption = isCompare ? getPercent((currentValue / prevDayMetric.value * 100 - 100) / 100) : 0;
  const compareOptionDifferenceNumber = isCompare ? currentValue - prevDayMetric.value : 0;
  const compareOptionDifference = label.includes('%') ? getPercent(compareOptionDifferenceNumber) : Math.round(compareOptionDifferenceNumber).toLocaleString();
  ...
  return (
    <div className="metric-card">
      ...
      <p className="metric-card__caption">Compared to {isCompare ? 'previous day' : affiliates.join(', ')}</p>
      ...
      {isCompare
        ? (
          <div className="metric-card__value-container">
            ...
            <div className={classNames("metric-card__right", {
              ...
              {!!compareOption && <p className="metric-card__compare"> {compareOptionDifference} ({compareOption}%)</p>}
            </div>
          </div>
        ) : (
          <>
            <h2 className={classNames("metric-card__value", {
