...
import { getAffiliatesDataCompare } from "../../helpers/api";
...
import { CohortsParams } from "../../types/ChartData";
...
export const ChartsContainer = () => {
  ...
  const [cohortParams, setCohortParams] = useState<CohortsParams>({
    ...
    divider: searchParams.get('divider') as DividerOptions || DividerOptions.EMPTY,
  ...
  useEffect(() => {
    ...
    setCohortParams((prevParams) => ({
      startDate: prevParams.startDate === startDate ? prevParams.startDate : startDate,
      endDate: prevParams.endDate === endDate ? prevParams.endDate : endDate,
      affiliates: compareArrays(prevParams.affiliates, affiliates) ? prevParams.affiliates : affiliates,
      divider: prevParams.divider === divider ? prevParams.divider : divider,
      metrics: compareArrays(prevParams.metrics, metrics) ? prevParams.metrics : metrics,
    }))
  }, [searchParams])
        ...
        <div className="charts-container__chart-container" key={metric}>
          ...
          <div className="charts-container__chart">
            <LineChart
              ...
              series={Object.entries(transformDataToChartFormat(compareAffiliates, metric, affiliates)).map(([affiliate, data]) => ({
                id: affiliate,
                label: affiliate,
                data: data,
                showMark: false,
              ...
              sx={{ width: '100%', height: '100%', fontSize: isLarge ? 300 : 300 }}
